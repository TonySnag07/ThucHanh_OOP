...
bool NgayThang::laNamNhuan() const {                                                           //Hàm kiểm tra năm nhuận
        return (iNam % 4 == 0 && iNam % 100 != 0) || (iNam % 400 == 0);
}

bool NgayThang::HopLe() {                                                                      //Hàm kiểm tra ngày có hợp lệ hay không
    if (iNam < 1 || iThang < 1 || iThang > 12) return false;                           
    if (iNgay < 1 || iNgay > SoNgay()) return false;
    return true;
}

int NgayThang::SoNgay() const {
...
void NgayThang::NgayThangTiepTheo() {                                                                    //Hàm tìm ngày tiếp theo
